...
Now we need to set up some coloring for the heatmap

```{r heatmapSetup}
exptCol  <- rep("black",nrow(mdata))
exptCol[mdata$experiment == "guideRNA"] = "red"
exptCol[mdata$experiment == "control"] = "green"

gRNAcol <- rep("black",nrow(mdata))
gRNAcol[mdata$guideID == "g349"] = "darkred"
gRNAcol[mdata$guideID == "g394"] = "pink"
gRNAcol[mdata$guideID == "g419"] = "green"

repCol <- as.vector(mdata$replicate)
repCol[repCol == 1] = palette()[1]
repCol[repCol == 2] = palette()[2]
repCol[repCol == 3] = palette()[3]
repCol[repCol == 4] = palette()[4]

head(data.frame(exptCol,gRNAcol,repCol))
```
...
```{r heatmap}
hcc = hclust(as.dist(1-cor(sigVst,method="pearson")),method="average")
heatmap(as.matrix(sigVst), col = colorRampPalette(c("red","blue"))(256), scale = "row", Rowv = NA, Colv = as.dendrogram(hcc), cexRow = 0.5, cexCol = 0.8, ColSideColors = exptCol)
legend("topleft",c("guide RNA", "control"), pch = 15, col = c("red", "green"),cex=0.5)
```
...
genes to see if there are "groups" of genes that have similar
expression across the samples. This time, I'm coloring by the replicates

```{r heatmap2}
hcc = hclust(as.dist(1-cor(sigVst,method="pearson")),method="average")
hcr = hclust(as.dist(1-cor(t(sigVst),method="pearson")),method="average")
heatmap(as.matrix(sigVst), col = colorRampPalette(c("red","blue"))(256), scale = "row", Rowv = as.dendrogram(hcr), Colv = as.dendrogram(hcc), cexRow = 0.5, cexCol = 0.8, ColSideColors = repCol)
legend("topleft",paste0("replicate",seq(1,4)), pch = 15, col = seq(1,4),cex=0.5)
```
