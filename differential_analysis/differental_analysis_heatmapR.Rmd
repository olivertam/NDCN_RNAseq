...
```{r heatmap}
hcc = hclust(as.dist(1-cor(sigGenes,method="pearson")),method="average")
heatmap(as.matrix(sigVst), color = colorRampPalette(c("red","blue"))(256), scale = "row", Rowv = NA, Colv = as.dendrogram(hcc), cexRow = 0.5, cexCol = 0.1, breaks = 0, ColSideColors = exptCol)
legend("topleft",c("guide RNA", "control"), pch = 15, col = c("red", "green"))
```
...
expression across the samples. This time, I'm coloring by guide RNA used

```{r heatmap2}
hcc = hclust(as.dist(1-cor(sigGenes,method="pearson")),method="average")
hcr = hclust(as.dist(1-cor(t(sigGenes),method="pearson")),method="average")
heatmap(as.matrix(sigVst), color = colorRampPalette(c("red","blue"))(256), scale = "row", Rowv = as.dendrogram(hcr), Colv = as.dendrogram(hcc), cexRow = 0.5, cexCol = 0.1, breaks = 0, ColSideColors = exptCol)
legend("topleft",c("g349", "g394", "g419 control"), pch = 15, col = c("darkred", "pink", "green"))
```
