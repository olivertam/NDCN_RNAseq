...
plot(voldata$log2FoldChange, voldata$log10FDR, pch = ".", col=volcolor, xlab = "Log2 fold change", ylab = "-log10 (FDR)")
legend("bottomright",c("differentially expressed features"), col=c("red"),pch = 20)
```

Here, we can now see the relationship between fold change and FDR.
...
```{r heatmap}
aheatmap(as.matrix(sigVst), color = "-RdBu:256", scale = "row", Rowv = NA, Colv = TRUE, legend = TRUE, cexCol = 0.5, cexCol = 0.1, breaks = 0, annCol = columnAnn, annRow = rowAnn, annColors = annColor)
```
