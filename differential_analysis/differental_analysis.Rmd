...
[variance-stabilizing transformation](https://en.wikipedia.org/wiki/Variance-stabilizing_transformation) (In essence, you are removing most
...
- [What if we have more than two conditions?](#multi_condition) 
   ...
   - Can we compare one condition to all others?
...
If you are trying to do a comparison between one
variable with two or more other variables, it is actually possible.

For example, if you didn't have the experimental condition in your
metadata, but would like to compare the effects of the two guide RNA
with the control RNA, you can do the following:

```{r 2guidesVg419}
res_gRNAvg419  <- results(dds_gRNA, contrast = list(c("guideIDg349","guideIDg394"),c("guideIDg419")), listValues = c(1/2, -1))
summary(res_gRNAvg419)
```

This is a little complicated to explain, but instead of providing the
condition name, test value and reference value, you can provide a list
of vectors containing the variables in the format of "condition_name +
value" (no spaces between them). You can then set up two vectors with
as many variables that you want to combine for your pairwise
comparison. In addition to the list, you need to provide weights for
the each pair in the list. In our case, since there are two values in
the first group, we assign each value as 1/2 of the total. Since the
total list values should equate to 0, the other side (with only one
control RNA) gets -1.

In theory, this should be equivalent to doing experiment vs control,
but there are slight variations since we are treating the two guideRNA
as independent observations (but manually combining them), whereas the
other approach would take them all into account.
...
```{r DESeqDataSet_GLM}
dds <- DESeqDataSetFromMatrix(countData = data, colData = mdata, design = ~ replicate + experiment)
cat("\n")
dds$experiment <- relevel(dds$experiment, ref = "control")
dds
```
...
In our case, we're just adding the replicate information as part of
our model, but we technically also have the guide RNA ID as
well. Thus, we could build a model that would also account for
variation in the guideRNA used if we wanted to:

``` {r example , eval = FALSE}
dds_GLM <- DESeqDataSetFromMatrix(countData = data, colData = mdata, design = ~ replicate + guideID + experiment)
```
...
```{r getCorrectedCounts}
vst_corr_counts <- assay(vst_corr)
```
